...
Type.forSchema = function (schema, opts) {
  ...
  if (typeof schema == 'string') { // Type reference.
    ...
    type = opts.registry[schema] || (opts.TypeRegistry && Type.forSchema(opts.TypeRegistry.resolve(schema), opts));
    if (type) {
  ...
  const schema2 = schema instanceof Object && schema.type instanceof Array ? schema.type : schema;

  if (Array.isArray(schema2)) { // Union.
    ...
    LOGICAL_TYPE = null;
    var types = schema2.map(function (obj) {
...
Type.prototype.fromBuffer = function (buf, resolver, noCheck) {
  ...
  buf.__$position = tap.pos < buf.length ? tap.pos : null;
  return val;
};
...
NullType.prototype._write = function (tap, val = null) {
  if (val !== null) {
...
StringType.prototype._write = function (tap, val, key, val2) {
...
function UnionType(schema, opts0) {
  var opts = opts0 || {};

  if(!opts.registry) opts.registry = {};

  Type.call(this, { name: schema.name }, opts);
...
function UnwrappedUnionType(schema, opts) {
  ...
  this._bucketIndices = {};
  this._struktur = opts.struktur;
  this._TypeRegistry = opts.TypeRegistry;
...
UnwrappedUnionType.prototype._getIndex = function (val) {
  var index = this._bucketIndices[getValueBucket(val, this)];
  if (this._dynamicBranches) {
...
UnwrappedUnionType.prototype._write = function (tap, val0 = null) {
  var val = global.__$Struktur && val0 instanceof global.__$Struktur ? val0.description : val0;
  
  var index = val && val.__$bucket ? val.__$bucket : this._getIndex(val);
...
WrappedUnionType.prototype._write = function (tap, val0) {
  var val = global.__$Struktur && val0 instanceof global.__$Struktur ? val0.description : val0;
  var index, keys, name;
  if (val === null) {
...
WrappedUnionType.prototype._copy = function (val = null, opts) {
  var wrap = opts && opts.wrap | 0;
  if (wrap === 2) {
...
MapType.prototype._write = function (tap, val0) {
  if (!val0 || (typeof val0 != 'object' && typeof val0 != 'function') || Array.isArray(val0)) {
    throwInvalidError(val, this);
  }
...
ArrayType.prototype._write = function (tap, val0) {
  var val = global.__$Struktur && val0 instanceof global.__$Struktur ? val0.description : val0;
...
RecordType.prototype._createConstructor = function (errorStackTraces) {
  ...
  innerBody += 'Object.assign(x, this);';
  innerBody += 'return x;';
 ...
 * @param val {...} Any value.
 */
function getValueBucket(val, union) {
  if (val === null) {
    return 'null';
  }
  var bucket = typeof val;
  if (bucket === 'object' || bucket === 'function') {
    if(union) {
      let i = 0;
      ...
      for (const type of types) {
        if (
          union._struktur &&
          union._TypeRegistry.resolve(union._struktur.type[i]).has(val) &&
          type.name
        ) return type.name;
